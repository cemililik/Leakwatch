...
| 4 | `twilio-api-key` | Trusted regional Twilio origin + Accounts probe | GET | Basic `{apiKeySID}:{apiKeySecret}` | Context required | P1 | `Raw` is now the explicitly labelled 32-character secret; `ExtraData` contains only its nearby non-secret `api_key_sid` and optional `account_sid`. Bare `SK...` SIDs are not findings. Production lacks trusted regional-origin wiring and makes no request. Accounts/Keys permissions differ for Main, Standard and Restricted keys, so `403` is inconclusive; only `401` on the selected region is inactive |
