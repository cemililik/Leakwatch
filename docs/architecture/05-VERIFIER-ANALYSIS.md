...
| 22 | `snyk-api-key` | Trusted regional/private `/rest/self` | GET | `Authorization: token {key}` | Context required | P1 | Snyk API origins are region/deployment-specific and are selected with the command-line-only `--verifier-origin` flag |
...
| 25 | `grafana-api-key` | Issuing instance `/api/access-control/user/permissions` | GET | `Bearer {token}` | Context required | P1 | Explicit `--verifier-origin` or backward-compatible `--grafana-instance-url`; no trusted URL means no request and `unverified`; repository-provided URLs are never trusted |
...
| 6 | `shopify-access-token` | Trusted store origin + `/admin/api/2026-07/graphql.json` | POST | `X-Shopify-Access-Token: {token}` | Context required | P1 | Uses a read-only `shop { name }` identity query with strict JSON/GraphQL response validation. Finding-controlled domains are ignored; the operator selects an exact `*.myshopify.com` origin with `--verifier-origin`. Only `401` on that store is inactive |
...
| 9 | `github-oauth-token` | Trusted GitHub.com/GHES subtype-specific identity endpoint | GET | `Bearer {token}` | Context required | P1 | `gho_`/`ghu_` → `/user`; `ghs_` → `/installation/repositories`; `ghr_` is never exchanged because verification would rotate it; `--verifier-origin` selects the issuer |
| 10 | `auth0-management-token` | Operator-trusted Auth0 tenant/custom `/api/v2/clients` | GET | `Bearer {token}` | Context required | P2 | JWT claims are untrusted scanner input and never choose the request origin; `--verifier-origin` provides explicit operator trust |
| 11 | `datadog-api-key` | Trusted Datadog site `/api/v1/validate` | GET | `DD-API-KEY: {key}` | Context required | P2 | Keys are site-bound; `--verifier-origin` accepts only exact official Datadog API hosts |
