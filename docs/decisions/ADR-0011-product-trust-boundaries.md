...
| Visual Studio Marketplace publication | **Experimental** | Manual `main`-only workflow, explicit boolean input, one reviewed VSIX artifact, environment-scoped `VSCE_PAT`, and a least-privilege metadata audit proving both the secret's environment provenance and externally configured reviewers/self-review/branch policy. No automatic tag publication. |
