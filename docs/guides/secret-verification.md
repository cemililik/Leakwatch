...
| GitLab PAT | `gitlab-pat` | Configure an operator-trusted GitLab.com or self-managed HTTPS origin. Only `glpat-` uses `/api/v4/user`; after active proof a best-effort self-metadata request adds validated scopes/expiry. Other recognized subtypes remain `unverified`. Only GitLab's standard invalid-token JSON `401` is inactive; DPoP challenges are inconclusive |
