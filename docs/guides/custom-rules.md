...
Leakwatch ships with **64 detectors (60 packages)** covering major cloud providers (AWS, GCP, Azure, DigitalOcean, Heroku, Vercel), AI platforms (OpenAI, Anthropic, Hugging Face, DeepSeek), CI/CD tools (GitHub, GitLab, CircleCI, Bitbucket), databases (PostgreSQL, MongoDB, Redis, Snowflake, RabbitMQ), and dozens of SaaS services. For secrets not covered by built-in detectors, you can define custom rules in YAML.
...
- **Long-tail SaaS services:** Niche or regional SaaS products not yet covered by the 64 detectors (60 packages)
