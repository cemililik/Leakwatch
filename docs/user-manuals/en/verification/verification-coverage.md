...
| `twilio-api-key` | Paired API Key SID plus an operator-trusted regional API origin (US1/IE1/AU1) | A bare `SK...` SID is a public identifier and is no longer reported. The detector emits an explicitly labelled 32-character API Key Secret only when a nearby `SK...` SID is present, with the secret in `Raw` and non-secret SIDs in context. Production has no trusted regional origin yet, so it makes no request and returns `unverified`; on a configured origin only `401` is inactive and permission `403` remains `verify_error`. |
