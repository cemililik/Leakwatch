...
| `twilio-api-key` | Eşleşen API Key SID ve operatörce güvenilen bölgesel API origin'i (US1/IE1/AU1) | Tek başına `SK...` SID herkese açık bir tanımlayıcıdır ve artık bulgu değildir. Dedektör, açıkça etiketlenmiş 32 karakterli API Key Secret'ı yalnızca yakınında `SK...` SID varsa; secret'ı `Raw`, gizli olmayan SID'leri context olarak üretir. Üretimde henüz güvenilir bölgesel origin yoktur; istek yapmadan `unverified` döner. Yapılandırılmış origin'de yalnız `401` inaktiftir, izin `403` yanıtı `verify_error` kalır. |
